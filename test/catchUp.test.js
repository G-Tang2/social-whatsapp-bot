...
test('buildCatchUpSummary describes a successful !in, !out, and !paid', () => {
  const summary = buildCatchUpSummary([
    { command: 'in', senderName: 'alex', added: ['Grace'], waitlisted: [], rejected: [] },
    { command: 'out', senderName: 'sam', removed: ['Henry'], rejected: [], promoted: [] },
    { command: 'paid', senderName: 'jo', paid: ['Quinn'], rejected: [] },
  ]);
  ...
  assert.match(summary.text, /• \*!in\* \(alex\): added Grace/);
  assert.match(summary.text, /• \*!out\* \(sam\): removed Henry/);
  assert.match(summary.text, /• \*!paid\* \(jo\): marked paid: Quinn/);
...
test('buildCatchUpSummary produces the exact bulleted, bold-command text layout', () => {
  ...
  assert.equal(
    ...
    'Caught up on 2 messages sent while I was offline:\n\n'
      + '• *!in* (alex): added Grace\n'
      + '• *!out* (sam): removed Henry'
  );
});

test('buildCatchUpSummary uses singular wording for exactly one entry', () => {
  const summary = buildCatchUpSummary([{ command: 'in', senderName: 'alex', added: ['Grace'], waitlisted: [], rejected: [] }]);
...
test('buildCatchUpSummary reflects a trailing "paid" keyword caught up on !in/!out (see commands/list.js runPaidIfFlagged)', () => {
  const summary = buildCatchUpSummary([
    { command: 'in', senderName: 'alex', added: ['Grace'], waitlisted: [], rejected: [], paid: ['Grace'], paidRejected: [], paidAmbiguous: null },
    { command: 'out', senderName: 'sam', removed: ['Henry'], rejected: [], promoted: [], paid: [], paidRejected: ['Henry - not on the payment-due list'], paidAmbiguous: null },
  ]);
  assert.match(summary.text, /• \*!in\* \(alex\): added Grace; marked paid: Grace/);
  assert.match(summary.text, /• \*!out\* \(sam\): removed Henry; couldn't mark paid: Henry - not on the payment-due list/);
});

test('buildCatchUpSummary reflects a caught-up "paid" combo even when the !in/!out half was a no-op (already on / no entry)', () => {
  const summary = buildCatchUpSummary([
    { command: 'in', senderName: 'alex', alreadyOn: ['Grace'], paid: ['Grace'], paidRejected: [], paidAmbiguous: null },
    { command: 'out', senderName: 'sam', noEntry: true, paid: [], paidRejected: [], paidAmbiguous: ['Sam1', 'Sam2'] },
  ]);
  assert.match(summary.text, /• \*!in\* \(alex\): already on the list as "Grace"; marked paid: Grace/);
  ...
  const summary = buildCatchUpSummary([
    {
      ...
      rejected: [],
      promoted: [
        { name: 'Henry', addedBy: 'sam@s.whatsapp.net' },
        { name: 'Quinn', addedBy: 'sam@s.whatsapp.net' }, // same adder as above - addedBy should only appear once in mentions
      ],
    },
  ]);
  assert.deepEqual(summary.mentions, ['sam@s.whatsapp.net']);
  assert.match(summary.text, /promoted off the waitlist: @sam \(Henry\), @sam \(Quinn\)/);
});
...
test('bufferCatchUpResult debounces: rapid entries for the same group flush together, once', async () => {
  ...
  bufferCatchUpResult(groupId, getSock, { command: 'in', senderName: 'alex', added: ['Grace'], waitlisted: [], rejected: [] });
  bufferCatchUpResult(groupId, getSock, { command: 'in', senderName: 'sam', added: ['Henry'], waitlisted: [], rejected: [] });
  ...
  assert.match(sent[0].content.text, /added Grace/);
  assert.match(sent[0].content.text, /added Henry/);
});
...
test('bufferCatchUpResult keeps different groups\' batches independent', async () => {
  ...
  bufferCatchUpResult(groupA, () => fakeSockA, { command: 'in', senderName: 'alex', added: ['Grace'], waitlisted: [], rejected: [] });
  bufferCatchUpResult(groupB, () => fakeSockB, { command: 'paid', senderName: 'jo', paid: ['Quinn'], rejected: [] });

  await new Promise((resolve) => setTimeout(resolve, 300));

  assert.equal(sentA.length, 2);
  assert.match(sentA[0].content.text, /added Grace/);
  assert.equal(sentB.length, 2);
  assert.match(sentB[0].content.text, /marked paid: Quinn/);
});
...
test('bufferCatchUpResult logs (without throwing) and keeps the batch buffered for a retry if no live socket is available at flush time', async () => {
  ...
  const sent = [];
  try {
    bufferCatchUpResult(groupId, () => null, { command: 'in', senderName: 'alex', added: ['Grace'], waitlisted: [], rejected: [] });
    await new Promise((resolve) => setTimeout(resolve, 300));
...
test('bufferCatchUpResult holds the flush open past the quiet-period timer while backlogSynced is false, even across two separate bursts', async () => {
  ...
  try {
    ...
    bufferCatchUpResult(groupId, getSock, { command: 'paid', senderName: 'sam', paid: ['Henry'], rejected: [] });
    await new Promise((resolve) => setTimeout(resolve, 300));
    ...
    assert.match(sent[0].content.text, /Caught up on 2 messages sent while I was offline/);
    assert.match(sent[0].content.text, /added Grace/);
    assert.match(sent[0].content.text, /marked paid: Henry/);
  } finally {
...
test('bufferCatchUpResult still flushes on the quiet-period timer alone when backlogSynced is (the default) true', async () => {
  ...
  bufferCatchUpResult(groupId, () => fakeSock, { command: 'in', senderName: 'alex', added: ['Grace'], waitlisted: [], rejected: [] });
  await new Promise((resolve) => setTimeout(resolve, 300));
