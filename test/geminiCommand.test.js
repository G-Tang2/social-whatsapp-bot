...
test('MAPPABLE_COMMANDS: includes every single command the bot has, no exceptions - plus "none"', () => {
  for (const cmd of ['in', 'out', 'paid', 'list', 'clear', 'clearpayments', 'newlist', 'date', 'location', 'courts', 'time', 'limit', 'allow', 'paymentlabel', 'regulars', 'exempt', 'undo', 'update', 'spamfilter', 'ai', 'help', 'admin', 'none']) {
  ...
  assert.equal(MAPPABLE_COMMANDS.length, 23);
});
...
test('interpretMessage: a compound message maps to multiple actions, returned in the order they should run', async () => {
  ...
  const result = await interpretMessage(
    'create a new list for next Sunday at Noble Park courts 1,2 at 7pm-9pm. Cap it at 12 and add Keith, Tu and Bao',
    { client }
  );
