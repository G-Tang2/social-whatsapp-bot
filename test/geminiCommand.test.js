...
test('MAPPABLE_COMMANDS: includes every single command the bot has, no exceptions - plus "none"', () => {
  for (const cmd of ['in', 'out', 'paid', 'list', 'clear', 'clearpayments', 'cancelsocial', 'newlist', 'date', 'location', 'courts', 'time', 'limit', 'allow', 'paymentlabel', 'regulars', 'exempt', 'courtcanceller', 'tournament', 'settournament', 'tournamentlimit', 'tournamentwinners', 'leaderboard', 'undo', 'update', 'inactivity', 'autonewlist', 'stale', 'spamfilter', 'welcome', 'ai', 'help', 'tips', 'admin', 'admintips', 'none']) {
  ...
  assert.equal(MAPPABLE_COMMANDS.length, 36);
});
...
test('interpretMessage: "cancelsocial" (e.g. calling off the whole social) is a valid mapped command, with no argument', async () => {
  const client = fakeClient(JSON.stringify({ actions: [{ command: 'cancelsocial', argText: '', confidence: 'high' }] }));
  const result = await interpretMessage("this week's social is cancelled, courts fell through", { client });
  assert.deepEqual(result, { actions: [{ command: 'cancelsocial', argText: '', confidence: 'high' }] });
});
