...
// the first part of a compound request (e.g. "create a new list ... cap it
// at 12 ... add Keith, Tu and Bao") ever took effect, because the old
// single-action shape could only dispatch one of the three. ---

test('e2e: a compound @-mention (new list + limit + add named people) dispatches all three actions in order', async () => {
  ai.setEnabled(GROUP_ID, true);
  setNextGeminiResponse({
    actions: [
      ...
      { command: 'in', argText: 'E2eCompoundKeith, E2eCompoundTu, E2eCompoundBao', confidence: 'high' },
    ],
  });
  fakeSockInstance.sentMessages.length = 0;

  await deliver(
    'create a new list for next Sunday at Noble Park courts 1,2 at 7pm-9pm. Cap it at 12. Add E2eCompoundKeith, E2eCompoundTu and E2eCompoundBao',
    { from: 'admin@s.whatsapp.net', type: 'notify', mentions: [BOT_JID] }
  );

  assert.equal(store.getCurrentEvent(GROUP_ID).limit, 12);
  const entries = store.getCurrentEvent(GROUP_ID).entries;
