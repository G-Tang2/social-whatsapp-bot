...
// returning the bare name. !update doesn't try to preserve TBC status from
// the pasted text either way (see store.js's applyListUpdate for why: a
...
// Returns { attendance, waitlist, duePayments, sectionsFound, headerLines }:
...
function parseListSections(text) {
  ...
  for (const rawLine of lines) {
    ...
    if (sectionsFound === 0 && !ATTENDANCE_HEADER.test(line) && !WAITLIST_HEADER.test(line)) {
  ...
  return { attendance, waitlist, duePayments, sectionsFound, headerLines };
}
