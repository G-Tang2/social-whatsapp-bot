...
// before this runs) and any trailing "(TBC)"/owed-since-date flags,
// returning the cleaned name. !update doesn't try to preserve TBC status
// from the pasted text either way (see store.js's applyListUpdate for why: a
// name kept from the current list keeps its ORIGINAL
// tbd/addedBy/addedByIsAdmin/owedSince metadata regardless of whether
// "(TBC)" (or the owed-since date) still appears next to it in the edited
// text - see that file for why the internal field is still named `tbd`,
...
//    appeared in the text, numbering and any trailing "(TBC)" flag stripped.
...
//    paste at all - "no signal, leave the label alone." `null` if
...
function parseListSections(text, referenceDate) {
  ...
  for (const rawLine of lines) {
    ...
    if (numberedMatch && section) {
      const name = parseEntryLine(numberedMatch[1]);
      if (name) {
        if (section === 'attendance') attendance.push(name);
  ...
  return { attendance, waitlist, duePayments, duePaymentsLabel, sectionsFound, headerLines };
}
