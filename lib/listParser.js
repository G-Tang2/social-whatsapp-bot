...
// Matches the "(paid)" flag formatList() (lib/helpers.js's
// formatEntryLine) appends to an attendance entry that's paid EARLY (see
// store.js's entry.paidEarly/markPaidEarly) - ahead of !newlist even
// archiving this cycle into duePayments yet. Same reasoning as
// TOURNAMENT_WL_SUFFIX above: has to stay in lockstep with
// formatEntryLine()'s own output, or a copy-pasted !update would treat
// "Chakriya (paid)" as a literal (and different) name. Deliberately
// STRIP-ONLY, same as TBC_SUFFIX - unlike "(🏆 WL)", nothing reads this
// back out into a settable field on !update (see parseEntryLine below);
// the tag is only ever set by an actual pay-early command, never by
// editing the pasted text, so a kept entry's existing paidEarly value
// (whatever it already was) is simply left alone regardless of whether
// this tag still appears next to it in the edit. formatEntryLine appends
// this INSIDE its own return value, and "(🏆 WL)" is then appended by ITS
// caller afterward (see formatList()'s Social-only rendering) - so for an
// entry with both, "(🏆 WL)" is actually the outermost/rightmost suffix in
// the real text ("Bao (paid) (🏆 WL)"), stripped first, before this one -
// see parseEntryLine below.
const PAID_EARLY_SUFFIX = /\s*\(paid\)\s*$/i;

// Strips a numbered entry's leading "N. " (already removed by NUMBERED_LINE
// before this runs) and any trailing "(TBC)"/"(🏆 WL)"/"(paid)"/owed-since-
// date flags, returning { name, tournamentWaitlisted }. A name kept from
// the current list keeps its ORIGINAL addedBy/addedByIsAdmin/owedSince/
// paidEarly metadata regardless of whether any of those tags still appear
// next to it in the edited text - see store.js's applyListUpdate - but
...
// unlike the others it isn't derived from anything else in the update; the
// tag itself is the only place that status appears in the pasted text at
// all.
function parseEntryLine(raw) {
  ...
  text = text.replace(PAID_EARLY_SUFFIX, '');
