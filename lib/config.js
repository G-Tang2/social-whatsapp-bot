...
// Whether a direct message to the bot gets the fixed DM_REDIRECT_REPLY
// (see index.js). OFF by default; set DM_REPLIES_ENABLED=true in .env to
// turn it on. Defaulting off avoids a footgun with running more than one
// deployment of this bot as separate linked companion devices on the SAME
// phone number (e.g. one deployment per WhatsApp group, all sharing one
// number rather than a dedicated number each) - WhatsApp delivers a DM to
// every companion device linked to that number, and each deployment is
// otherwise a fully independent process (its own AUTH_DIR/DATA_DIR, no
// shared state) with no way to know the others already replied to the
// same DM, so leaving this on everywhere by default would mean anyone
// running more than one deployment gets a burst of duplicate replies
// unless they remember to turn it off on all but one. Turn it on
// explicitly on whichever single deployment should actually answer DMs -
// group commands don't have this problem at all, since ALLOWED_GROUPS
// already scopes those per deployment.
const DM_REPLIES_ENABLED = String(process.env.DM_REPLIES_ENABLED || '').toLowerCase() === 'true';
