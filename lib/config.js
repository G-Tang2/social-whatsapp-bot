...
// (see index.js). On by default; set DM_REPLIES_ENABLED=false in .env to
// silence it for a specific deployment. Exists for the case of running
// more than one deployment of this bot as separate linked companion
// devices on the SAME phone number (e.g. one deployment per WhatsApp
// group, all sharing one number rather than a dedicated number each) -
// WhatsApp delivers a DM to every companion device linked to that number,
// and each deployment is otherwise a fully independent process (its own
// AUTH_DIR/DATA_DIR, no shared state) with no way to know the others
// already replied to the same DM. Turning this off on all but one
// deployment is the fix - group commands don't have this problem at all,
// since ALLOWED_GROUPS already scopes those per deployment.
const DM_REPLIES_ENABLED = String(process.env.DM_REPLIES_ENABLED || 'true').toLowerCase() !== 'false';
