...
// Optional, deliberately-outside-the-repo "banter" treatment for one
// specific real person (see index.js's handleMessage) - a random chance
// of an eye-roll reaction instead of the usual tick, plus a snarky
// one-liner (see BANTER_LINES below). Unlike everything else in this
// file, this identifies one specific real person by their WhatsApp
// JID/LID and singles them out for mockery, so it belongs in your own
// gitignored .env, never in committed source - see the git history for
// why this was built this way instead of hardcoded. Unset by default -
// the whole feature is a no-op until both this AND BANTER_LINES are
// configured.
const BANTER_TARGET_JID = process.env.BANTER_TARGET_JID || '';

// Pipe-separated ("|") list of snarky one-liners for the BANTER_TARGET_JID
// treatment above - one is picked at random each time it fires. Mix in
// as many/few digs at their badminton skills as you like; there's no
// separate list for those, so how often they come up is just however
// often you include them here. Empty/unset means no comment gets sent
// (the reaction still swaps, but no text follows).
const BANTER_LINES = (process.env.BANTER_LINES || '').split('|').map((s) => s.trim()).filter(Boolean);
