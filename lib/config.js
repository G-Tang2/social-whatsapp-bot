...
// message that's actually asking people to go sign up, as opposed to the
// 26-hour court-canceller heads-up, which is a private nudge to a specific
// person rather than a pitch to join. Same image for every group this bot
// moderates (not per-group) - unset by default, in which case that
// broadcast stays plain text, unchanged from before this existed. Read
// fresh from disk each time it's sent (see lib/vacancyReminder.js), not
// cached at startup, so swapping the file takes effect on the very next
// warning without needing to restart the bot.
