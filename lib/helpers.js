...
const { PAYMENT_LABEL, MAX_NAMES_PER_COMMAND, COMMAND_PREFIX } = require('./config');
...
// Matches the plain-English word "and" used as a separator between names,
// e.g. "Luke and Peter" or the Oxford-comma form "Luke, Peter, and Grace" -
// normalized into an ordinary comma (with the SAME `,?` swallowing an
// already-present Oxford comma right before it, so that doesn't survive as
// its own empty token) before every name-list below gets comma-split, so a
// raw TYPED command accepts the same natural phrasing the AI path already
// produces on its own (see lib/geminiCommand.js's COMMAND_ARG_GUIDE - the
// model is told to always emit comma-separated argText, so this normalizing
// step only ever matters for a raw typed command's argText, never an
// AI-mapped one). Whitespace-delimited on both sides deliberately - "Andy"/
// "Sandra"/"Andrew" are untouched, since "and" only counts as this
// separator when it's its own word, not a substring inside a real name.
function normalizeAndSeparator(text) {
  return text.replace(/,?\s+and\s+/gi, ', ');
}

// Splits a raw comma-and/or-"and"-separated list into trimmed, non-empty
// tokens, e.g. "Grace, Henry and Iris" or "Grace, Henry, and Iris" both
// become ["Grace", "Henry", "Iris"] (see normalizeAndSeparator above).
// Shared by every name-list command below (!in/!out/!paid via parseNames,
// !regulars, !exempt, !tournamentwinners, !newlist's "with ..." clause,
// and "me and +N" via commands/list.js's matchMeAndPlusN) so they all
// accept the same separators consistently, rather than each re-splitting
// on a bare "," of its own.
function splitCommaList(text) {
  return normalizeAndSeparator(text || '')
    .split(',')
    .map((t) => t.trim())
    .filter(Boolean);
}
...
// - "Grace and Henry"/"Grace, Henry and Henry+1" work identically, via
// splitCommaList's "and" normalization above. A bare command with no
// argument falls back to a single-element list containing the sender's own
// name.
//
...
function parseNames(argText, fallbackName) {
  ...
  const rawTokens = splitCommaList(argText);
...
// A literal "@Snoopy" TYPED as plain text - not a real, JID-based mention
// selected from WhatsApp's own contact picker. A genuine mention's raw
// text is always "@" followed by the mentioned person's PHONE NUMBER
// (WhatsApp only renders the saved contact name "Snoopy" client-side, for
// display - see stripMentionTokens' own doc comment above) - it can never
// literally contain the word "Snoopy" itself. So this can only ever match
// someone who typed the bot's name by hand (e.g. their WhatsApp client
// didn't register the picker selection, or they just typed it out of
// habit) rather than genuinely mentioning it - see index.js's
// messageMentionsBot() for where this is treated as equivalent to a real
// mention anyway, and stripMentionTokens below for why it's stripped the
// same way a real mention token is before anything reaches the AI.
// Deliberately NOT global (no /g/ flag) - this exact object is reused for
// repeated .test() calls (see messageMentionsBot()), and a global regex's
// .test()/.exec() is stateful across calls via lastIndex, which would
// silently start missing matches after the first call that ever matched.
// stripMentionTokens below makes its own freshly-global-flagged copy of
// this same pattern for its one-shot .replace() instead of sharing this
// object directly, for exactly that reason.
const LITERAL_BOT_MENTION_REGEX = /@snoopy\b/i;
...
function stripMentionTokens(text, mentionedJids) {
  ...
  cleaned = cleaned.replace(new RegExp(LITERAL_BOT_MENTION_REGEX, 'gi'), ' ');
  cleaned = cleaned.replace(/[ \t]+/g, ' '); // collapse horizontal whitespace only
  cleaned = cleaned.replace(/[ \t]*\n[ \t]*/g, '\n'); // trim spaces hugging a line break
  return cleaned.trim();
}

// Real bug report: quoting someone's own "@Snoopy ..." mention back (every
// reply/tagged notification in this codebase passes `{ quoted: msg }` to
// sock.sendMessage) shows WhatsApp's raw mention text - always
// "@<phone number>" (see stripMentionTokens' own doc comment above), never
// the friendly display name - as the quote preview above the bot's own
// message, for anyone who hasn't saved that number as a contact. Nothing
// is functionally broken, but it's a confusing "@21200473..." instead of
// "@Snoopy".
//
// Confirmed from Baileys' own source (Utils/messages.js): the `quoted`
// option's `.message` field is copied wholesale into the outgoing
// message's own contextInfo.quotedMessage - a snapshot WE control, not a
// live lookup of the original message - so building a version of `msg`
// with mention tokens stripped from its TEXT (but its `key` left
// untouched, so tapping the quote still correctly jumps to the real
// original message) changes only what's shown inline in the preview.
// Every `{ quoted: msg }` call site across this codebase should pass
// `{ quoted: buildQuotePreviewMsg(msg) }` instead. Returns the original
// `msg` unchanged when there's nothing to strip, to avoid needlessly
// rebuilding the common case (typed commands, whose raw text never
// contains a mention).
function buildQuotePreviewMsg(msg) {
  const text = getMessageText(msg);
  if (!text) return msg;
  const stripped = stripMentionTokens(text, getMentionedJids(msg));
  if (stripped === text) return msg;
  return { ...msg, message: { conversation: stripped } };
}
...
function formatPromotedMessage(promotedEntries) {
  ...
  return {
    text: `Off the waitlist — you're in! 🎉\n${lines.join('\n')}`,
...
// Announcement sent when an admin cancels the current social outright (see
// store.js's cancelSocial() and commands/admin.js's !cancelsocial) - tags
// everyone who was on the attendance list or waitlist right before it got
// wiped, same one-line-per-entry/addedBy-based tagging (and same known
// non-self-entry limitation - see formatPromotedMessage's own doc comment
// above) as every other roster-tagging message in this file. Returns null
// (nothing to send) if nobody was on the list at all - the caller falls
// back to a plain text reply instead.
function formatCancelledSocialMessage(cancelledEntries) {
  if (!cancelledEntries || !cancelledEntries.length) return null;
  const lines = cancelledEntries.map((e) => `@${e.addedBy.split('@')[0]} — ${e.name}`);
  return {
    text: `❌ This week's social has been cancelled.\n${lines.join('\n')}`,
    mentions: cancelledEntries.map((e) => e.addedBy),
  };
}
...
function formatList(groupId) {
  ...
  // Cancelled banner (see store.js's cancelled/cancelSocial() and
  // commands/admin.js's !cancelsocial) - shown above everything else,
  // including the winners banner below, so it's the first thing anyone
  // scanning a reposted list sees. The Attendance/waitlist sections
  // underneath still render normally (empty, per cancelSocial() wiping
  // them) - this banner is what actually tells the reader WHY, since an
  // empty list alone looks just like a brand new one.
  if (event.cancelled) {
    sections.push(`❌ *This social has been cancelled* - hang tight for the next ${COMMAND_PREFIX}newlist.`);
  }
