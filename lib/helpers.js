...
const { COMMAND_PREFIX, PAYMENT_LABEL, MAX_NAMES_PER_COMMAND } = require('./config');

// Plain-text divider printed above the payment-due section header in
// formatList() below, to set it apart from Attendance/Waitlist at a
// glance. Deliberately plain (no bold asterisks) - see the comment at its
// use site.
const SECTION_DIVIDER = '──────────';

// Matches a bare "+N" token, e.g. "+1" or "+ 2" - shorthand for "myself and
// N unnamed friends" (see parseNames below). Deliberately distinct from an
// explicit "Sam+1"-style name (that's a real name with a literal "+1"
// suffix typed by whoever's adding Sam, not this shorthand) - this only
// matches a token that is ENTIRELY "+" then digits, nothing else.
const PLUS_N_TOKEN = /^\+\s*(\d+)$/;

// Splits "!in Alex, Sam, Sam+1" style input into ["Alex", "Sam", "Sam+1"].
// A bare command with no argument falls back to a single-element list
// containing the sender's own name.
//
// Also expands a bare "+N" token (e.g. "!in +2", or "@bot add me and 2
// friends" mapped by the AI to argText "+2" - see lib/geminiCommand.js's
// COMMAND_ARG_GUIDE) into `fallbackName`, `${fallbackName}+1`, ...,
// `${fallbackName}+N` - i.e. "myself and N unnamed friends", reusing the
// existing "Name+1" convention (see the README/help text) but generated
// automatically instead of requiring the sender to type it out per guest.
// Works the same way for !in/!out/!paid, since all three call this - "!out
// +2" removes yourself and both your +1/+2 entries by the same expansion.
// A non-"+N" token (a real name, or an explicit "Sam+1") passes through
// unchanged. See commands/list.js's handleIn for the one place this
// matters beyond simple name-matching: it separately figures out that the
// FIRST (bare fallbackName) entry from this expansion is really "the
// sender", so it's recorded with the correct `self` flag - the `+1`/`+2`
// entries are treated as separate people the sender is vouching for, same
// as if they'd typed "Sam, Sam+1" for someone else.
...
// `["Extra Guest", "regular players"]` with a stored roster of `["Peter",
// "Chris"]` becomes `["Extra Guest", "Peter", "Chris"]`. Shared by
...
function maxNamesReply(verb, suffix = '') {
  return `You can ${verb} up to ${MAX_NAMES_PER_COMMAND} names${suffix} in one command (no limit for group admins).`;
}
...
// argument text, e.g. "!in paid" (yourself) or "!in paid Alex, Sam" (a
...
// tournament Alex, Sam", "!in tournament paid Alex, Sam", or either alone.
...
// creates it, e.g. "!newlist 20/08 EBC | 13-18 | 8PM start with Peter,
// Chris, Linda" or, with no location/courts/time mentioned at all, just
// "!newlist 20/08 with Peter, Chris, Linda".
//
...
// names would make "20/08 | | | Peter, Chris" (admin didn't mention
...
function formatTournamentPromotedMessage(promotedEntries) {
  ...
  const lines = promotedEntries.map((e) => `@${e.addedBy.split('@')[0]} — ${e.name}`);
  return {
    text: `🏆 A tournament spot opened up — you're in!\n${lines.join('\n')}`,
    mentions: promotedEntries.map((e) => e.addedBy),
  };
}

// Renders one attendance entry as its numbered line, e.g. "3. Peter (TBC)" -
// shared by formatList()'s plain and tournament-aware Attendance rendering
// below, and by formatTournamentRoster() (also below), so the "(TBC)"
// suffix convention stays in exactly one place.
function formatEntryLine(entry, number) {
  return `${number}. ${entry.name}${entry.tbd ? ' (TBC)' : ''}`;
}

// Renders JUST the "🏆 Tournament (n/limit)" block (header, a pointer to
// !tournament for the rules text, plus numbered roster - no
...
//
// Displayed as "(TBC)" (renamed from the old "(TBD)" label) for any entry
// with `tbd: true` - the internal field/data-shape name is unchanged (see
// store.js) since it's never shown to users and renaming it would mean
// migrating already-persisted data/lists.json files for no user-visible
// benefit; only this display string, and everywhere else that surfaces it
// to a human (replies, !update's pasted-list parsing, README/help text),
// changed. See lib/listParser.js's TBC_SUFFIX for the matching parse side.
function formatList(groupId) {
  ...
  // and commands/admin.js's !tournamentwinners) - shown above everything
  // else, including the date/location header, same as it'd read as a pinned
  // announcement. Only shown while the tournament sub-feature itself is
  // still on - if an admin turns it off, the banner (and the rest of the
  // tournament UI below) goes quiet along with it, even though the winners
  // themselves are still remembered underneath in case it's turned back on.
  ...
  if (!event.entries.length && !event.tournamentEnabled) {
    ...
    sections.push(`*Attendance*\n\n(empty${limitNote} - use ${COMMAND_PREFIX}in to add your name)`);
    ...
    sections.push(`*Attendance* ${formatCount(event.entries.length, event.limit)}\n\n${body}`);
  } else {
  ...
  if (waitlist.length) {
    const waitLines = waitlist.map(
      (entry, i) => `${i + 1}. ${entry.name}${entry.tbd ? ' (TBC)' : ''}`
    );
  ...
  if (due.length) {
    ...
    // tag one at a time. Each group is a plain (non-bold) date header line
    // (dates.js's formatDisplayDate(), e.g. "20th Aug Thu") followed by
    // that group's names, renumbered from 1; dated groups are sorted MOST
    // RECENT first (the freshest debt at the top, the longest-overdue one
    // at the bottom). An entry with no owedSince at all - predates this
    // feature, or was added straight into the payment section by hand via
    // !update rather than carried over automatically - gets its own
    // "No date" group, placed BEFORE every dated group regardless of this
    // ordering (same wording as the "No date set" header line used when
    // the event itself has no date - see NO_DATE_SET_LINE in
    // lib/listParser.js). These group headers are plain text, not bold, so
    // listParser.js's parseListSections() (powering !update) never tries
    // to interpret them - they just fall through its tolerant stray-text
    // skip on a copy-pasted !update, same as any other commentary line; no
    // parser changes were needed to support this grouping. (Older,
    // already-sent messages may still show the previous inline
    // "Name (9th Aug Sun)" format - see OWED_SINCE_SUFFIX in
    ...
    // ISO (YYYY-MM-DD) strings sort chronologically as plain strings; reversed
    // so the most recently-owed-since date comes first, oldest last.
    const sortedDates = [...datedGroups.keys()].sort().reverse();

    const groupBlocks = [];
    if (noDateGroup.length) {
      groupBlocks.push(`No date\n${noDateGroup.map((entry, i) => `${i + 1}. ${entry.name}`).join('\n')}`);
    }
    for (const isoDate of sortedDates) {
      const entries = datedGroups.get(isoDate);
      groupBlocks.push(`${formatDisplayDate(isoDate)}\n${entries.map((entry, i) => `${i + 1}. ${entry.name}`).join('\n')}`);
    }
    ...
    // REAL payment header (`*${label}*` right after it) is still what
    // actually triggers that detection.
