...
// store.js's addCourts()), same idea as stripLeadingPaidKeyword above but
// with two accepted spellings for the one flag. Both words are recognized
// as synonyms specifically because natural-language phrasing (via the AI
// command feature - see lib/geminiCommand.js) is just as likely to say "I
// got extra courts 12-14" as "add courts 12-14"; the bot doesn't need to
// distinguish them, just recognize either as "merge, don't replace." Only
// strips ONE match at the front - there's only one flag here, so nothing
// to keep looping for.
//
...
function formatList(groupId) {
  ...
  // is visually set apart from whatever's above it every time, not just in
  // some branches.
