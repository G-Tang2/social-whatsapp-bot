...
// Renders one attendance entry as its numbered line, e.g. "3. Peter" -
// shared by formatList()'s plain and tournament-aware Attendance rendering
// below, and by formatTournamentRoster() (also below). A "(paid)" tag is
// appended for anyone who's settled up EARLY (store.js's entry.paidEarly/
// markPaidEarly - see commands/list.js's markPaidAnywhere) - ahead of
// !newlist even archiving this cycle into duePayments yet, so there'd be
// no other visible sign of it otherwise. Never appears for a waitlist
// entry in practice (paying early isn't offered for a spot you don't
// have yet - see markPaidEarly's own doc comment), but reads from the
// same field either way, so this stays correct even if that ever
// changes. Appended HERE, inside this shared function - "(🏆 WL)" is
// then appended AFTER it by formatList()'s own Social-only rendering
// (the one place both can apply to the same entry), so the real combined
// order is "Bao (paid) (🏆 WL)", "(🏆 WL)" outermost - see
// PAID_EARLY_SUFFIX/TOURNAMENT_WL_SUFFIX in lib/listParser.js, which have
// to stay in lockstep with this exact shape (and strip in the matching
// reverse order) so a copy-pasted !update round-trips both tags
// correctly instead of treating either as part of the literal name.
function formatEntryLine(entry, number) {
  return `${number}. ${entry.name}${entry.paidEarly ? ' (paid)' : ''}`;
}
