...
async function sendFirstWarning(sock, groupId, vacancies) {
  ...
  const text = `@all 📢 *${vacancies} ${spotWord}* still open! Plenty of room for you (and your friends) - sign up soon, or we'll have to cancel the courts tomorrow.`;
...
async function sendCancelWarning(sock, groupId, vacancies, cancellerJid) {
  const spotWord = vacancies === 1 ? 'spot' : 'spots';
  const text = `⏰ Still *${vacancies} ${spotWord}* open. @${cancellerJid.split('@')[0]} - looks like we may need to cancel some courts.`;
