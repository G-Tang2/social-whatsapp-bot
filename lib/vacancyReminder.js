...
// without a restart) - returns the Buffer, or null if unset, missing, or
// unreadable, in which case sendFirstWarning below just falls back to a
// plain-text message like it always has. Logged (not thrown) on failure -
// a bad image path should never be the reason the actual warning doesn't
// go out.
function readReminderImage() {
  if (!VACANCY_REMINDER_IMAGE_PATH) return null;
  try {
    return fs.readFileSync(VACANCY_REMINDER_IMAGE_PATH);
  } catch (err) {
...
async function sendFirstWarning(sock, groupId, vacancies) {
  ...
  const image = readReminderImage();
  await sock.sendMessage(
    groupId,
    image ? { image, caption: text, mentions: participantIds } : { text, mentions: participantIds }
  );
}
