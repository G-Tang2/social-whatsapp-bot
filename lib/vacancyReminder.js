...
async function sendFirstWarning(sock, groupId, vacancies) {
  ...
  const tags = participantIds.map((id) => `@${id.split('@')[0]}`).join(' ');
  const spotWord = vacancies === 1 ? 'spot' : 'spots';
  const text = `📢 ${vacancies} ${spotWord} still open with 48 hours to go! Plenty of room - sign up now or courts may be cancelled.\n\n${tags}`;
...
async function sendCancelWarning(sock, groupId, vacancies, cancellerJid) {
  const spotWord = vacancies === 1 ? 'spot' : 'spots';
  const text = `⏰ Still ${vacancies} ${spotWord} open with 26 hours to go. @${cancellerJid.split('@')[0]} - please cancel the courts if this doesn't fill up in time.`;
