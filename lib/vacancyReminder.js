...
//   - 50 hours out, 6+ vacant spots: @-mentions the WHOLE group, asking
//     people to sign up or courts may be cancelled - with an image
//     attached if VACANCY_REMINDER_IMAGE_PATH is set (see
//     readReminderImage() below), plain text otherwise.
...
const { ALLOWED_GROUPS, TIMEZONE, VACANCY_REMINDER_IMAGE_PATH } = require('./config');
...
// implements: mention everyone at 50 hours out, mention the designated
// court-canceller at 26 hours out, both gated on 6+ vacant spots.
const MIN_VACANCIES = 6;
const FIRST_WARNING_HOURS = 50;
...
// Reads VACANCY_REMINDER_IMAGE_PATH fresh off disk (not cached at
// startup, so swapping the file takes effect on the very next warning
// without a restart) - returns { buffer, isVideo }, or null if unset,
// missing, or unreadable, in which case sendFirstWarning below just falls
// back to a plain-text message like it always has. Logged (not thrown) on
// failure - a bad path should never be the reason the actual warning
// doesn't go out. `isVideo` (from the file extension - see
// VIDEO_EXTENSIONS above) picks the message shape sendFirstWarning sends
// it as: `video` + `gifPlayback: true` for a looping "GIF"-style clip, or
// plain `image` for anything else (jpg/png/etc).
function readReminderMedia() {
  if (!VACANCY_REMINDER_IMAGE_PATH) return null;
  try {
    const buffer = fs.readFileSync(VACANCY_REMINDER_IMAGE_PATH);
    const isVideo = VIDEO_EXTENSIONS.has(path.extname(VACANCY_REMINDER_IMAGE_PATH).toLowerCase());
    return { buffer, isVideo };
  } catch (err) {
    console.error(`[vacancyReminder] Couldn't read VACANCY_REMINDER_IMAGE_PATH ("${VACANCY_REMINDER_IMAGE_PATH}") - sending the warning as plain text instead:`, err.message);
    return null;
  }
}
...
async function sendFirstWarning(sock, groupId, vacancies) {
  ...
  const text = `@all 📢 *${vacancies} ${spotWord}* still open! Plenty of room for you (and your friends) - sign up soon, or I'll have to cancel the courts tomorrow, and nobody wants that.`;
  const media = readReminderMedia();
  let content;
  if (!media) {
...
async function sendCancelWarning(sock, groupId, vacancies, cancellerJid) {
  const spotWord = vacancies === 1 ? 'spot' : 'spots';
  const text = `⏰ Still *${vacancies} ${spotWord}* open. @${cancellerJid.split('@')[0]} - looks like you may need to cancel some courts, unless a miracle happens.`;
