...
function describeOut(entry) {
  ...
  if (entry.tournamentPromoted && entry.tournamentPromoted.length) {
    // Same idea, but for the (🏆 WL) tournament queue (see store.js's
    // promoteFromTournamentWaitlist) - a distinct queue from the main
    // waitlist above, so called out separately.
    const tagged = entry.tournamentPromoted.map((e) => `@${e.addedBy.split('@')[0]} (${e.name})`).join(', ');
    parts.push(`promoted into the tournament: ${tagged}`);
  }
  parts.push(...paidParts);
  if (!parts.length) parts.push('nothing to do');
  return `*!out* (${entry.senderName}): ${parts.join('; ')}`;
...
function buildCatchUpSummary(entries) {
  ...
  for (const entry of entries) {
    ...
    if (entry.tournamentPromoted && entry.tournamentPromoted.length) {
      for (const p of entry.tournamentPromoted) {
        if (!mentions.includes(p.addedBy)) mentions.push(p.addedBy);
      }
    }
  }
