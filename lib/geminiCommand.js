...
const MAPPABLE_COMMANDS = [
  ...
  'limit', 'allow', 'paymentlabel', 'regulars',
  'tournament', 'settournament', 'tournamentlimit', 'tournamentwinners',
  'undo', 'update', 'inactivity', 'stale', 'spamfilter', 'ai', 'help', 'admin',
  'none',
];
...
const COMMAND_ARG_GUIDE = `- "in": join/sign up. argText = other specific people named (comma-separated, e.g. "Peter, Chris"), or "" for the sender themselves. Never include the sender's own name. SPECIAL CASE - unnamed friends: if the sender wants themselves plus N friends WITHOUT naming those friends (e.g. "add me and 2 friends", "add me and +2", "sign up me plus 3"), argText = "+N" (just a plus sign and the number, e.g. "+2") - this is shorthand the bot expands into the sender's own entry plus N "+1"/"+2"/... guest entries automatically, so do NOT write out names yourself for this case. Only use "+N" when no specific names are given for the friends - if names ARE given (e.g. "add me, Peter, and Chris"), list them normally instead. IMPORTANT: "+N" ALWAYS means "N MORE guests than whatever the sender already has" (the bot works this out from the current list itself, continuing the numbering) - if the CURRENT LIST already shows the sender with existing "+1"/"+2"/etc guest entries and they ask for N MORE, still just output "+N" (the count they asked for now), NEVER a bigger number trying to account for what's already there yourself - e.g. sender already has "Jordan+1..Jordan+3" and asks to "add 3 more friends": argText = "+3", not "+6". SPECIAL CASE - the group's saved "regular players": if the sender wants to sign up the group's saved regulars (see the REGULAR PLAYERS line below and the "regulars" command), e.g. "add the regular players" or "put the regulars down", include the literal text "regular players" as one of the comma-separated items in argText (e.g. "regular players" alone, or "regular players, Extra Guest" alongside an explicitly named person) - do NOT enumerate the stored names yourself even though you can see them, the bot expands this token from the real, current, saved list. SPECIAL CASE - joining the tournament too: if the sender ALSO wants to opt into the group's tournament sub-feature (see the "🏆 Tournament" section of the CURRENT LIST, if present, and the "settournament" command below) at the same time as joining, e.g. "put me down for the tournament too", "sign up Peter for the tournament", PREPEND the literal word "tournament" to the very FRONT of argText, before any names (e.g. "" -> "tournament" for the sender alone, or "tournament, Peter" for someone named plus the tournament flag - NOT "Peter, tournament", which the bot no longer recognizes as the flag) - the bot recognizes that literal word as a flag only when it leads, not a person's name. SPECIAL CASE - bare "<name> social only" with no verb at all (e.g. "Garvin social only", just a name plus that phrase): check the CURRENT LIST first. If that name is NOT currently listed under "🏆 Tournament" (they're not on the list at all yet, or already under "Social only"), this is a plain add request - argText = just the name(s), comma-separated, WITHOUT the "tournament" prefix - "social only" here is just describing the outcome the sender wants (on the social list, not the tournament), not a request to move anyone out of the tournament, since they were never in it to begin with. (If the name IS currently under "🏆 Tournament", this phrasing instead means taking them out of the tournament - that's "out"'s own matching SPECIAL CASE below, not "in".)
- "out": leave/be removed. argText = same rules as "in", including the "+N" shorthand for "remove me and my N unnamed friends/guests" - but NOT the "regular players" token, which "out" doesn't support (bulk-removing a whole saved roster is too risky to shorthand) - if the sender wants the regular players removed, spell out actual names instead, or fall back to "low" confidence if you can't. SPECIAL CASE - leaving the tournament only, NOT the whole list: if the sender wants someone taken OUT of the tournament (or off its (🏆 WL) queue) while staying on the social list - e.g. "move Garvin to social only", "take Garvin out of the tournament", "Garvin isn't playing the tournament anymore", "Garvin's just social now" - this is NOT the same as removing them from the list entirely, so PREPEND the literal word "tournament" to the very FRONT of argText, before any names (e.g. "" -> "tournament" for the sender alone, or "tournament, Garvin" for someone named - NOT "Garvin, tournament", which the bot no longer recognizes as the flag), exactly mirroring "in"'s tournament-opt-in special case above. Only use plain "out" (no "tournament" flag) when the request is genuinely about leaving the list/event entirely. This SPECIAL CASE (including a bare "<name> social only" with no verb, e.g. "Garvin social only") only applies when that name is CURRENTLY listed under "🏆 Tournament" in the CURRENT LIST - there has to be an actual tournament spot (or 🏆 WL queue slot) to take them out of. If the name isn't on the list at all yet (or is already under "Social only"), don't use "out" for that phrasing at all - it means adding them normally instead, which is "in"'s own matching SPECIAL CASE above, not this one.
- "paid": mark payment done. argText = same rules as "in", including the "+N" shorthand - but NOT the "regular players" token either, same reasoning as "out" above.
...
- "regulars" (ADMIN to change, anyone can view): manage the group's saved "regular players" roster - the same saved list "regular players" refers to for "in"/"newlist" above. argText = the exact names, comma-separated, to REPLACE the whole saved roster with, when the sender is declaring/stating who the regulars are (e.g. "these people are regular players: Harry, Bonny, Ron" -> argText = "Harry, Bonny, Ron", a plain list of names or a numbered list following the request both count); or "add <names>" / "remove <names>" when the sender clearly means adding to/removing from the EXISTING roster rather than replacing it wholesale (e.g. "Dean is a regular player too" -> argText = "add Dean"; "Chris isn't a regular anymore" -> argText = "remove Chris"); or "clear" to empty the roster entirely; or "" (no argument) to just view the current roster. Don't confuse this with "in"/"newlist"'s "regular players" TOKEN (a placeholder that means "use the saved roster") - "regulars" argText is the opposite: it's what DEFINES that roster, so it always contains real names (or add/remove/clear), never the literal token itself.
- "tournament" (anyone, view only): show the tournament rules text an admin has set (see "settournament"'s "rules" sub-case below) - NOT who's currently opted in, and NOT the on/off toggle, both of which now live under "settournament" below. argText = "" (always). Use this for a request specifically about the RULES ("what are the tournament rules", "how does the tournament work") - not for "who's in the tournament" (that's bare "settournament") or "is the tournament on" (also bare "settournament").
- "settournament" (ADMIN to change, anyone can view): turn the group's tournament sub-feature on/off, view who's currently opted in, or set the rules text "tournament" (above) shows. argText = "on" or "off" for an explicit request to turn it on/off; "" (no argument) to just view who's currently opted in/whether it's on; or "rules " followed by the exact rules text, e.g. "rules Best of 3, single elimination, losers bracket at 1PM" for an explicit request to SET/change the tournament rules ("the tournament rules are...", "set the rules to...", "tournament is best of 3 now"). Don't confuse this with "in"'s tournament-opt-in special case above - "settournament" here is about the FEATURE itself (on/off/view/rules), not one person joining it.
- "tournamentlimit" (ADMIN): change the tournament's own headcount cap - separate from the main "limit" above, which caps the whole social Attendance list. argText = a whole number, or "off" to remove the cap entirely.
- "tournamentwinners" (ADMIN): set the two-name "Congrats to X and Y for winning last week's tournament" banner shown above the list. argText = exactly two names, comma-separated, e.g. "Irfan, Tu" for "congrats to Irfan and Tu on winning" or "Irfan and Tu won the tournament". Only use this for an explicit statement of who WON - not a guess, and not for anything about who's currently playing (that's "in"'s tournament flag, or just viewing "tournament").
...
MULTIPLE ACTIONS: a single message can bundle more than one distinct request together, e.g. "create a new list for next Sunday at Noble Park courts 1,2 at 7pm-9pm. The tournament limit is 12. Add Keith, Tu and Bao to the tournament" is genuinely THREE separate requests - starting a new list, capping the tournament, and adding specific people to it - even though it's one message. Return one "actions" entry PER DISTINCT REQUEST, in the ORDER they should be applied:
- Only split into multiple actions when the message actually contains SEPARATE requests that map to DIFFERENT commands (or the same command used for genuinely unrelated sub-requests). Do NOT split a single request just because it mentions multiple people or details - "remove Peter and Chris" is still ONE "out" action with argText "Peter, Chris", not two; "create a list for Sunday at Noble Park" is still ONE "newlist" action with the location folded into argText, not two.
- Order matters when one action depends on another - most commonly, a brand new list has to exist (a "newlist" action) before anyone can be added to it or its settings changed, so a "newlist" action always comes FIRST if the message has one, even if it wasn't the first thing said.
- If the sender wants to both start a new list AND add specific people to the tournament on it in the same message, do NOT try to cram the tournament-flagged names into "newlist"'s own "with ..." clause (that clause only adds people to the plain social list, it has no tournament tagging of its own) - instead, leave those names out of the "with ..." clause (or omit "with ..." entirely if every named person is tournament-bound) and add a SEPARATE "in" action afterward with the "tournament" keyword leading argText, e.g. actions: [{"newlist", "23/08 Noble Park | 1, 2 | 7pm-9pm"}, {"tournamentlimit", "12"}, {"in", "tournament, Keith, Tu, Bao"}] for the example above. Names the sender wants on the list WITHOUT the tournament flag can still go in "newlist"'s own "with ..." clause as usual.
