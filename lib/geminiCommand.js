...
- "tournamentwinners" (ADMIN): set the two-name "Congrats to X and Y for winning last week's tournament" banner shown above the list. argText = exactly two names, comma-separated, e.g. "Irfan, Tu" for "congrats to Irfan and Tu on winning" or "Irfan and Tu won the tournament". Only use this for an explicit statement of who WON - not a guess, and not for anything about who's currently playing (that's "in"'s tournament flag, or just viewing "tournament"). IMPORTANT - "newlist" clears the winners banner (it's an announcement about the cycle that just ended, not a standing setting): if the message ALSO starts a new list in the same breath as announcing winners (e.g. "create a new list for tomorrow, the tournament winners are Peter and Rob"), the "newlist" action must still run FIRST per the MULTIPLE ACTIONS ordering rule below, with "tournamentwinners" as a SEPARATE action right after it - never fold a "the winners are..." announcement into the same message as "newlist" without adding this second action, or the winners never actually get recorded.
...
- Order matters when one action depends on another - most commonly, a brand new list has to exist (a "newlist" action) before anyone can be added to it or its settings changed, so a "newlist" action always comes FIRST if the message has one, even if it wasn't the first thing said. This matters even more than usual for "tournamentwinners" specifically, since "newlist" actively CLEARS the winners banner (see "tournamentwinners"'s own entry above) - e.g. "@Snoopy create a new list for tomorrow and the tournament winners are Peter and Rob" is actions: [{"newlist", "<tomorrow's DD/MM>"}, {"tournamentwinners", "Peter, Rob"}], "newlist" first so the winners get set on the FRESH list rather than immediately wiped by it.
