...
// new list for Sunday, cap it at 12, and add Keith/Tu/Bao to it"), and each
// one maps to its own {command, argText, confidence} item, in the order
// they should be dispatched - see SYSTEM_PROMPT's "MULTIPLE ACTIONS" rules
// below and index.js's handleAiMention, which runs each high-confidence
// action through the exact same handler a typed command would hit,
// sequentially (so a later action can see an earlier one's effect, e.g.
// "!in" after the "!newlist" that created the list it's joining). An
// ordinary single-request message still comes back as a one-item array -
// there's no separate flat shape to special-case.
