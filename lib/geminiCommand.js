...
- COLLECTIVE NAME REFERENCES (in/out/paid): a request naming a GROUP by a shared trait rather than one exact name - "all the Harrys", "everyone named Alex", "both Toms" - means EVERY entry that's actually ELIGIBLE for the command (see each command's own scope below - e.g. "paid" is only ever eligible within the payment-due section, never Attendance/Waitlist) and matches the name, not just the first one or two that catch your eye. Build the match list by scanning ONLY the lines actually inside the eligible section/sub-group(s) - never start from "every line in the CURRENT LIST containing this name" and filter after; a bare, exact-spelled match sitting in an INELIGIBLE section (e.g. "all the Harrys paid" - a plain "Harry" plainly visible up in Attendance) is not a candidate at all, not even a rejected one, and must never end up in argText no matter how obviously it matches the stated name. Scan EVERY eligible line, not just entries sitting next to each other - matches are often scattered, separated by unrelated names in between, or split across "paid"'s multiple dated sub-groups (see its own entry below) - so don't stop checking once you've found a plausible-looking cluster; keep going to the end of every eligible section/sub-group. List every genuine match, comma-separated, exactly as spelled in the CURRENT LIST.
...
- "courtcanceller" (ADMIN to change, anyone to view): who gets tagged with a reminder to cancel the courts if the list is still well short of people close to the social's start time. argText = "" to view; "off"/"clear"/"none" to turn it off. To SET it, the sender must @-mention that person directly in the SAME message (e.g. "@Snoopy make @Alex the court canceller") - a typed name alone can't be reliably tagged, and argText itself is IGNORED for this case (the real handler reads the message's own @-mention, never argText - same reasoning as "update" below) - put the person's name in argText anyway, just NOT empty and NOT "off"/"clear"/"none" (an empty argText means "view", not "set"). If the request is clearly asking to set someone but nobody was actually @-mentioned in the message, use "low" confidence instead of guessing who they meant.
