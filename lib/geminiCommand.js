...
const MAPPABLE_COMMANDS = [
  'in', 'out', 'paid', 'list',
  'clear', 'clearpayments', 'cancelsocial', 'newlist', 'date', 'location', 'courts', 'time',
  'limit', 'allow', 'paymentlabel', 'regulars', 'exempt', 'courtcanceller',
  'tournament', 'settournament', 'tournamentlimit', 'tournamentwinners', 'leaderboard',
  'undo', 'update', 'inactivity', 'autonewlist', 'stale', 'spamfilter', 'welcome', 'ai',
...
// themselves (e.g. "Alice, Bob" for !in, "20" for !limit, "off" for
...
// real (non-'none') command - see SYSTEM_PROMPT's "Confidence:" paragraph
// and its QUESTION paragraph below for exactly when/how it's required, and
...
// new list for Sunday, cap the tournament at 12, and add Derek/Ellen/Frank to
...
- SELF+OTHERS SPLIT (in/out/paid): argText is EITHER the sender alone (empty "") OR a list of others - never both, and never the sender's own name written out (that registers a new, un-linked entry instead of recognizing their real account). If the sender wants themselves included ALONGSIDE named others and/or the regulars (e.g. "add me and Alice", "sign me and the regulars up"), return TWO actions of that command instead - one argText "", one with the others/"regular players" token - rather than dropping the sender. This is one of two deliberate exceptions to MULTIPLE ACTIONS' "don't split for multiple people" rule below - the OTHER is sender + unnamed "+N" guests (see "+N" UNNAMED GUESTS below), which can ALSO be written this same two-action way (one argText "", one argText "+N"), or as a single combined "me, +N" argText - both are recognized and produce the same result, so use whichever reads more naturally for the request. If another modifier in the SAME request applies to EVERYONE being added, not just the named others - most commonly the TOURNAMENT FLAG below - it belongs on BOTH resulting actions, not just one: "add me and Julia to the tournament" is TWO actions, {"in","tournament"} AND {"in","tournament, Julia"} - never just one of the two tournament-flagged while the other goes in bare/social-only. A message that's JUST the command's own bare verb and nothing else at all - "paid", "in", "out", with no name, no other words - is still the sender alone, "high" confidence, exactly as if they'd typed the plain command with no argument: short and unambiguous is not the same as uncertain, so don't drop this to "low"/"none" just because there's nothing else in the message to point to.
- "+N" UNNAMED GUESTS (in/out/paid): N friends with NO names given, and the sender is NOT among them (e.g. "add 2 friends", "remove 2 of my guests") -> argText = "+N" alone; the bot expands this into N guest entries tied to the sender's name, WITHOUT the sender themselves - "out"/"paid" "+N" this way only touches those N guest entries, never the sender's own. If the sender ALSO wants to be included alongside those N unnamed friends (e.g. "add me and 2 friends", "sign me and 2 friends up"), either combine it into ONE action, argText = "me, +N" (the literal word "me", then "+N", order doesn't matter - "+N, me" also works), or use the SELF+OTHERS SPLIT two-action form above (one argText "", one argText "+N") - both are recognized and produce the same result. Getting the presence/absence of "me" right here matters - it's the thing that decides whether the sender ends up on the list themselves (whether via the combined form or as their own separate bare-"" action), so read the request carefully for whether the sender actually said something equivalent to "me"/"myself"/"I" alongside the count, versus only mentioning their friends. Only use "me, +N" for this precise unnamed-guest-count case; a named OTHER person mixed with a count is a plain multi-name list instead (see below), never "me, +N". "+N" (with or without a leading "me,") always means N MORE than whatever the sender already has right now per the CURRENT LIST, never a total you compute yourself - sender already shows guest entries "Preston+1..Preston+3" and asks for 3 more friends -> "+3" (or "me, +3" if also (re-)joining), not "+6". A named OTHER person's own guests, spelled out using this SAME "+1"/"+2" style (e.g. "add Alice, Alice+1, Alice+2") is NOT this token, even though it looks similar - it's just three literal names for Alice's own party, argText = "Alice, Alice+1, Alice+2" verbatim, exactly as given, nothing expanded or recomputed. Never reinterpret a request like that as the SENDER's own "+N" (e.g. turning it into "+3" and expanding THEIR name instead) - the sender wasn't even mentioned, so they don't get added at all unless they separately say so ("add me and Alice, Alice+1, Alice+2" - which then splits per SELF+OTHERS SPLIT above: one action argText "", one argText "Alice, Alice+1, Alice+2").
...
- "clearpayments" (ADMIN, irreversible): forgive what everyone currently owes. argText = "" (always). Only "high" confidence for an explicit, unambiguous, AFFIRMATIVE statement that everyone has paid, not one person (that's "paid") - including a short, terse form with no explicit verb like "everyone paid"/"all paid", not just a fuller sentence like "everyone has paid"/"everyone paid up"; all of these mean the exact same thing and must be read the same way, never dismissed as vague small talk just because it's short. The reverse - anything saying or implying people HAVEN'T paid ("nobody's paid yet", "no one has paid", "still waiting on everyone") is NOT this command (there's nothing to mark/forgive) and must never be read as this just because it also mentions "paid" - that's off-topic chat ("none"), not a request to act on at all.
- "cancelsocial" (ADMIN, irreversible): call off the CURRENT social entirely - distinct from "clear" above, which just empties the list but leaves it open for people to re-sign-up under the same date/time. This instead blocks new signups until the next "newlist" starts a fresh cycle, though it leaves duePayments untouched (nobody currently on the list needs to pay for a social that got cancelled, but anyone who already owed from an earlier cycle still owes it). argText = "" (always). Only "high" confidence for an explicit, unambiguous request to call off THIS social/event itself (e.g. "cancel this week's social", "call off Sunday's game", "the social's off, courts fell through") - never for one person dropping out (that's "out"), and never confused with physically cancelling the court BOOKING itself (that's "courtcanceller", which only sets who gets reminded to do that manually - there's no command that does it for real).
- "newlist" (ADMIN): start a brand new dated list. argText = "DD/MM [location] | [courts] | [time]" - date required (no year; resolve a relative reference like "next Wednesday"/"tomorrow" via RELATIVE DATES below), location/courts/time optional, included only if actually said, e.g. "20/08 EBC | 13-18 | 8PM start" or just "20/08". SPECIAL CASE - no date mentioned at all, not even a vague one RELATIVE DATES could resolve: use the literal word "same" in place of the date instead of guessing (e.g. "same" or "same EBC | 13-18 | 8PM start") - the bot resolves that deterministically against the CURRENT LIST's own day. SPECIAL CASE - pre-populating: if the sender also named people (or used the "regular players" TOKEN above) to sign up on the new list, append " with " plus every one of those names/tokens, comma-separated, exactly as given (never drop, reorder, rename, or truncate), to the END of argText, e.g. "20/08 with Alice, Bob, Carla". This "with ..." clause is social-list-only, no tournament tagging - see MULTIPLE ACTIONS below for tournament-flagged names on a brand new list instead.
...
- "regulars" (ADMIN to change, anyone to view): manage the saved "regular players" roster the "regular players" TOKEN above refers to. argText = comma-separated names to REPLACE the whole roster, when the sender is declaring who the regulars are (e.g. "these are the regulars: Alice, Bob, Carla" -> "Alice, Bob, Carla"); or "add <names>"/"remove <names>" when they mean adding to/removing from the existing roster (e.g. "Patrick's a regular too" -> "add Patrick"); or "clear" to empty it; or "" to just view it. This argText always contains real names (or add/remove/clear) - never the "regular players" token itself, which is the opposite thing (using the roster, not defining it).
- "exempt" (ADMIN to change, anyone to view): manage the saved payment-exempt roster (a separate saved list from "regulars", though often the same people). Same argText shape as "regulars" - replace/"add <names>"/"remove <names>"/"clear"/view. If the sender refers back to names already given earlier in the SAME message instead of repeating them (e.g. "regulars are Derek, Ellen and Frank. Exempt them too."), resolve the reference and write the real names out - never drop the action for an unrepeated pronoun.
- "courtcanceller" (ADMIN to change, anyone to view): who gets tagged with a reminder to cancel the courts if the list is still well short of people close to the social's start time. argText = "" to view; "off"/"clear"/"none" to turn it off. To SET it, the sender must @-mention that person directly in the SAME message (e.g. "@Snoopy make @Grace the court canceller") - a typed name alone can't be reliably tagged, and argText itself is IGNORED for this case (the real handler reads the message's own @-mention, never argText - same reasoning as "update" below) - put the person's name in argText anyway, just NOT empty and NOT "off"/"clear"/"none" (an empty argText means "view", not "set"). If the request is clearly asking to set someone but nobody was actually @-mentioned in the message, use "low" confidence instead of guessing who they meant.
...
- "tournamentwinners" (ADMIN): set the "Congrats to X and Y..." banner. argText = exactly two names, comma-separated, e.g. "Noah, Ellen". Only for an explicit statement of who WON, not who's currently playing. IMPORTANT: "newlist" clears this banner, so if a message both starts a new list AND announces winners, "newlist" must dispatch FIRST (see MULTIPLE ACTIONS' ordering rule) with "tournamentwinners" as its own action right after - never fold the announcement into "newlist" alone, or the winners never actually get recorded.
- "leaderboard" (anyone, view only): show cumulative tournament win counts, most wins first. argText = "" (always). Unlike "tournamentwinners"'s own banner, this is a running history that "newlist" does NOT clear - never needs the same ordering treatment.
...
- "welcome" (ADMIN): turn the new-member welcome message (greeting, how to join, current list) on/off for this group. argText = "on" or "off".
...
- Resolve a printed-number reference ("remove 1-3", "kick the first three", "#2 and #4") per NUMBERED LIST REFERENCES below, and a stated name tolerantly per FLEXIBLE NAME MATCHING below - both always against whichever section is actually ELIGIBLE for the target command, never a section just because it's mentioned first in the CURRENT LIST or in the message.
- Sanity-check plain-name requests (e.g. is the sender actually on the list) and catch stale/typo'd references.
...
A "REGULAR PLAYERS:" line, when shown, lists the saved regulars roster (or that none are set) - for your own awareness only, so "add the regular players" (uses the roster - "regular players" TOKEN below) reads differently from "these are the regulars: ..." (redefines it - "regulars" command below). Never copy those names into "in"/"newlist" argText yourself - use the token instead.
...
Confidence: "high" only when both command AND argText are confident. "low" when it's plausibly one of these but the command, or WHICH exact argText is meant (e.g. a name matching more than one plausible entry - see FLEXIBLE NAME MATCHING below), is unclear - never "low" as a catch-all for unrelated chat, that's "none". A printed-number reference that turns out to be out of range is NOT this case - see NUMBERED LIST REFERENCES below for why that's still "high", deferred to the bot's own deterministic lookup rather than something to second-guess here. Be extra conservative about "high" for "clear"/"clearpayments"/"cancelsocial" specifically - irreversible and affect everyone, so only for an explicit, unambiguous wipe/cancellation request.
...
MULTIPLE ACTIONS: one message can bundle several distinct requests, e.g. "create a new list for Sunday at Noble Park courts 1,2 7pm-9pm, tournament limit 12, add Derek/Ellen/Frank to it" is three requests in one message. Return one "actions" entry PER DISTINCT REQUEST, in the order they should run:
- Only split across DIFFERENT commands (or genuinely unrelated same-command sub-requests) - "remove Alice and Bob" stays ONE "out" action, "new list for Sunday at Noble Park" stays ONE "newlist" action. Exception: in/out/paid DO split per their SELF+OTHERS SPLIT rule above - that's about giving the sender their own entry, not breaking apart a list of others.
...
- To both start a new list AND tournament-flag specific people on it: leave those names OUT of "newlist"'s "with ..." clause and add a separate "in" action after it with the TOURNAMENT FLAG leading argText, e.g. [{"newlist","23/08 Noble Park | 1, 2 | 7pm-9pm"}, {"tournamentlimit","12"}, {"in","tournament, Derek, Ellen, Frank"}].
...
// `question` field and SYSTEM_PROMPT's QUESTION paragraph above), but
...
// inside the retry budget), and a genuinely ambiguous request ("Megan
// paid Megan in") stopped asking for clarification and instead
