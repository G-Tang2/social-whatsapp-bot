...
shape, the bot sends a short reply pointing you to @-mention it instead -
most hand-edits like this are really just someone trying to add/remove
themselves or mark themselves paid, and mentioning it directly lets it
figure out which of those you meant, rather than you having to pick the
right command yourself.
...
Every reply string in this codebase is hand-written directly in Snoopy's
own voice (the *Peanuts* character: the World War I Flying Ace fantasy,
the never-finished novel typed on the doghouse roof, Woodstock, root
beer, supper time, quietly pleased with himself) - see `commands/*.js`
and `index.js`. Sent as plain text, with no live Gemini call involved.

Gemini itself is reserved for one job only: the natural-language mapping
call (see "Natural-language commands" above). The one place a live
Gemini-authored reply reaches a sender at all is the `offTopicReply` that
same call produces for a genuinely off-topic message (small talk, an
unrelated question) - already written in Snoopy's voice per that call's
own system prompt. Every dispatched command's confirmation, by contrast,
stays fully hand-written and never touches Gemini a second time - keeping
the bot's Gemini usage, cost, and failure surface limited to the one call
