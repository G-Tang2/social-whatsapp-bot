...
const ADMIN_HELP_TEXT = [
  ...
  `• *${COMMAND_PREFIX}leaderboard* - cumulative tournament win counts, most wins first (anyone can view; only ${COMMAND_PREFIX}tournamentwinners adds to it, there's no direct way to set it)`,
  '',
  '*Group settings*',
  `• *${COMMAND_PREFIX}spamfilter [on|off]* - auto-delete stock/crypto spam in this group, ON by default (admins to change)`,
  `• *${COMMAND_PREFIX}welcome [on|off]* - greet new members, explain how to join, and show the current list, ON by default (admins to change)`,
  ...
  `• *${COMMAND_PREFIX}inactivity [on|off]* - inactivity reminders for this group, OFF by default (admins to change - see ${COMMAND_PREFIX}admintips for the full behavior)`,
...
const ADMIN_TIPS_TEXT = [
  ...
  '',
  `_• Add "with name1, name2, ..." to the end of ${COMMAND_PREFIX}newlist to sign everyone up on the new list at once, e.g. ${COMMAND_PREFIX}newlist 20/08 with Alice, Bob, Carla - respects the limit, waitlisting anyone over it like ${COMMAND_PREFIX}in would_`,
  '',
  ...
  '',
  `_• ${COMMAND_PREFIX}cancelsocial calls the whole cycle off, unlike ${COMMAND_PREFIX}clear: it still empties Attendance/Waitlist, but also blocks new ${COMMAND_PREFIX}in signups against it until ${COMMAND_PREFIX}newlist starts a genuinely fresh one, and @-mentions everyone who was signed up so nobody has to find out by trying to ${COMMAND_PREFIX}in. Payment-due entries are untouched either way - nobody on THIS list owes for it, but earlier unpaid cycles still stand_`,
  '',
  '*Payments*',
  ...
  '',
  `_• ${COMMAND_PREFIX}leaderboard tracks wins forever - unlike ${COMMAND_PREFIX}tournamentwinners's own "Congrats..." banner, which ${COMMAND_PREFIX}newlist wipes every cycle, the leaderboard just keeps adding up. Only ${COMMAND_PREFIX}tournamentwinners adds to it (one win each, per name, every time it's set) - there's no way to edit or reset someone's count directly_`,
  '',
  '*Group settings*',
  ...
  '',
  `_• ${COMMAND_PREFIX}welcome is per-group, ON by default - greets whoever joins (tagged), explains how to ${COMMAND_PREFIX}in, and posts the current list; several people joining together get ONE combined message, not one each, and I never welcome myself when first added to a group. ${COMMAND_PREFIX}welcome off to go quiet_`,
  '',
  ...
  '',
  `_• Vacancy warnings need a real start time to work from - set ${COMMAND_PREFIX}time to something with an actual time in it (e.g. "8PM start"), not just a description. With 6+ spots still open: I @-mention the WHOLE group once, 52 hours before start ("sign up or courts may be cancelled"); then, if still 6+ open, I @-mention whoever ${COMMAND_PREFIX}courtcanceller names once more, 26 hours before start, as their own reminder to actually go cancel. Each only fires once per list - a fresh ${COMMAND_PREFIX}newlist resets both. No ${COMMAND_PREFIX}courtcanceller set means that second reminder just never goes out_`,
  '',
  ...
  '',
  `_• ${COMMAND_PREFIX}inactivity is per-group, OFF by default - once on, I track when each member last sent ANY message here (not just commands), and tag anyone quiet for ${INACTIVITY_WARN_AFTER_DAYS}d+ with a one-time reminder (admins exempt). ${COMMAND_PREFIX}stale (admins only) lists everyone currently warned, flagging anyone past ${INACTIVITY_REMOVE_AFTER_DAYS}d since their warning as OVERDUE. I never remove anyone myself - that's always a human admin's call from WhatsApp's own group UI. Any message from someone clears their warning and resets the clock. Turning it on (re)seeds everyone with a fresh "just seen" baseline, so nobody's flagged for quiet time from before it was on_`,
  '',
  '*Other*',
