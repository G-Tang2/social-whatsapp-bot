...
const ADMIN_TIPS_TEXT = [
  ...
  '',
  `_• Vacancy warnings need a real start time to work from - set ${COMMAND_PREFIX}time to something with an actual time in it (e.g. "8PM start"), not just a description. With 6+ spots still open: I @-mention the WHOLE group once, 50 hours before start ("sign up or courts may be cancelled"); then, if still 6+ open, I @-mention whoever ${COMMAND_PREFIX}courtcanceller names once more, 26 hours before start, as their own reminder to actually go cancel. Each only fires once per list - a fresh ${COMMAND_PREFIX}newlist resets both. No ${COMMAND_PREFIX}courtcanceller set means that second reminder just never goes out_`,
  '',
